...
},
{
  ...
  const [allProducts, setAllproducts] = useState(products[0]);
       ...
       <div className="absolute inset-0 bg-cover bg-center transition-all duration-500">
        ...
        />
          ...
          <div className="relative z-5 text-center pt-32">
            <h1 className="absolute mx-80 text-center mb-10 font-bold text-4xl">{allProducts.text}</h1>
          
             <div className="flex justify-center gap-12 mt-20">
          {products.map((item, i) => (
            <div
              key={i}
              onMouseEnter={() => setAllproducts(item)}
            ...
            >
              <Image
                ...
                height={250}
                alt={item.title}
                className="rounded-full border-2 border-gray-500 hover:border-blue-500 transition-colors bg-gray hover:shadow-[0_0_35px_rgba(43,127,255,0.8)]"
              />
